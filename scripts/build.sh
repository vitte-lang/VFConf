...
verify_build_outputs() {
  ...
  for binary in main check dump fmt; do
    ...
    path="$(find_built_binary "$binary")" ||
      ...
      die "compiled executable unavailable: ${binary}.exe"
  ...
  log "Compiled executables verified"
...
write_build_metadata() {
  ...
  then
    ...
    if git -C "$PROJECT_ROOT" diff \
        --quiet --ignore-submodules HEAD -- 2>/dev/null
...
export_distribution() {
  ...
  copy_executable \
    main \
    "${destination}/bin/vfconf${suffix}"

  copy_executable \
    check \
    "${destination}/bin/vfconf-check${suffix}"

  copy_executable \
    dump \
    "${destination}/bin/vfconf-dump${suffix}"

  copy_executable \
    fmt \
    "${destination}/bin/vfconf-fmt${suffix}"
...
main() {
  ...
  # Packaging only occurs after all requested validation gates pass.
...
echo "[OK] scripts/build.sh syntax valid"